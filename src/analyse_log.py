...
def analyse_log(path_to_file):
    ...
    with open("data/mkt_campaign.txt", "w") as txt_file:
        txt_file.write(
            f"{mais_pedido(object_info['maria'], valid_keys['ingredients'])}\n"
        )
        txt_file.write(
            f"{comida_por_pessoa(object_info['arnaldo'],'hamburguer')}\n"
        )
        txt_file.write(
            f"{dias_pratos(object_info['joao'], valid_keys['ingredients'])}\n"
        )
        txt_file.write(
            f"{dias_pratos(object_info['joao'], valid_keys['days'])}\n"
        )
